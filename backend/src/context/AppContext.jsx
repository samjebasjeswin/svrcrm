...
export function AppProvider({ children }) {
  ...
  const getLinkedEntryDisplayValue = useCallback((pageId, entryId, displayFieldName) => {
  ...
  }, [getPageEntries, pages, currentCompanyId]);

  const getInboundLinks = useCallback((targetPageId, targetEntryId) => {
    const inboundLinks = [];
    ...
    return inboundLinks;
  }, [currentCompanyId, pages, getPageEntries, getLinkedEntryDisplayValue]);
  ...
  const addPageLinks = (links) => {
    ...
    const newLinksBatch = links.map(l => ({ ...l, id: Date.now() + Math.random(), batchId }));
  ...
  const updatePageLink = (id, linkData) => {
    setPageLinks(prev => {
      ...
      if (targetPage) {
        ...
        updatePage(link.targetPageId, { headings: updatedHeadings });
      }
      ...
      value={{
        ...
        submitExternalForm: (companyId, type, data) => {
          ...
          if (!targetPage && typeStr === 'contact') {
            ...
            setPages((prev) => ({ ...prev, [cid]: [...(prev[cid] || []), targetPage] }));
          }
