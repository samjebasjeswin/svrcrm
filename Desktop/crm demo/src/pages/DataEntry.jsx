...
export default function DataEntry() {
    ...
    const { getPage, addEntry, updateEntry, getPageEntries, getLinkedEntryDisplayValue } = useApp();
    ...
    const renderFieldInput = (heading, sub, field, rowIdx = null) => {
                ...
                <div className="data-entry-field-input-wrapper" style={{ display: 'flex', gap: '8px' }}>
                    ...
                    {linkedEntries.length === 0 && (
                        <div className="link-empty-hint">
                            ...
                            <button
                                className="btn btn-ghost btn-sm"
                                onClick={() => navigate(`/data-entry/${field.linkedPageId}`)}
                            >
        ...
        const inputProps = {
            value,
            onChange: (e) => handleChange(heading.id, sub.id, field.id, e.target.value, field.maxChars),
        ...
        <>
            ...
            <div className="data-entry-header">
                <div className="data-entry-header-left">
                    <button className="back-btn" onClick={() => navigate(`/data-entry/${pageId}`)}>
                        ←
                    </button>
                    <div className="breadcrumb">
                        <span style={{ fontWeight: 700 }}>{page.name}</span>
                        <span className="separator">›</span>
                        <span style={{ color: 'var(--text-secondary)' }}>{entryId ? 'Edit Entry' : 'New Entry'}</span>
                    </div>
                </div>
                <div className="data-entry-header-right">
                    <button className="btn btn-outline btn-sm" onClick={() => navigate(`/data-entry/${pageId}`)}>
                        Back to List
                    </button>
                    <button className="btn btn-outline btn-sm" onClick={() => navigate(`/edit-page/${pageId}`)}>
                        ✏️ Edit Structure
                    </button>
                </div>
            </div>
            ...
            <div className="data-entry-body" key={refreshKey}>
                ...
                {headings.length === 0 && (
                    <div className="data-entry-empty-page">
                        ...
                        <button className="btn btn-primary" onClick={() => navigate(`/edit-page/${pageId}`)}>
                            Edit Page Structure
                        </button>
                    </div>
                )}
